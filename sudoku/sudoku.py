...
def group(values: tp.List[T], n: int) -> tp.List[tp.List[
    T]]:  # создаетcя новый список, в котором каждый элемент списка является результатом некоторой операции, примененной к каждому элементу
